# Exercises the evidence workflow's core decisions without substituting for managed integrations.
...
def test_review_payload_skips_unneeded_review_under_relaxed_policy() -> None:
    ...
    assert domain.review_payload(
        base_state(review_policy="relaxed", claims=[claim(confidence=0.2)])
    ) is None
...
def test_reconcile_synthesize_and_validate_preserve_evidence(monkeypatch) -> None:
    ...
    state = base_state(
        questions=["Question"],
        evidence_batches=[
            {"question_index": 0, "claims": [claim()], "chunks": []}
        ],
    )
...
def test_resume_and_failure_state_use_stable_database_contracts() -> None:
    ...
    assert sum(
        "insert into run_events" in sql for sql, _ in failure_engine.connection.calls
    ) == 1
