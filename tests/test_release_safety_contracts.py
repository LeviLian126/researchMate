...
def test_qdrant_rerank_projection_indexes_every_filter_field() -> None:
    ...
    source = (ROOT / "scripts" / "provision_qdrant_rerank.py").read_text(encoding="utf-8")
    ...
    completed_log = source.index("Qdrant rerank backfill {BACKFILL_VERSION} already verified")
...
def test_workflow_claim_versions_are_serialized_and_incremented() -> None:
    ...
    source = (
        (ROOT / "workers" / "ai-worker" / "src" / "researchmate_worker" / "workflow_commit.py")
        .read_text(encoding="utf-8")
        .lower()
    )
...
def test_report_refresh_filters_changed_documents_and_preserves_unaffected_sections() -> None:
    ...
    workflow = "\n".join(
        (ROOT / "workers" / "ai-worker" / "src" / "researchmate_worker" / filename).read_text(
            encoding="utf-8"
        )
