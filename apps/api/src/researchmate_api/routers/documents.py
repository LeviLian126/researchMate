...
def create_upload_url(
    ...
    response = store.create_upload_url(user, payload)
    if response is None:
...
def create_document(
    ...
    document = store.create_document(user, payload)
    if document is None:
        raise_api_error(status.HTTP_404_NOT_FOUND, "PROJECT_NOT_FOUND", "Project was not found.")
    return document
...
def list_project_documents(
    ...
    documents = store.list_project_documents(user, project_id)
...
def get_document(document_id: UUID, user: CurrentUser = Depends(get_current_user)) -> DocumentRecord:
    document = store.get_document(user, document_id)
    if document is None:
...
def complete_upload(
    ...
    job = store.complete_document(user, document_id, extracted_text)
    if job is None:
        raise_api_error(status.HTTP_404_NOT_FOUND, "DOCUMENT_NOT_FOUND", "Document was not found.")
...
def delete_document(document_id: UUID, user: CurrentUser = Depends(get_current_user)) -> dict[str, str]:
    job = store.delete_document(user, document_id)
    if job is None:
        raise_api_error(status.HTTP_404_NOT_FOUND, "DOCUMENT_NOT_FOUND", "Document was not found.")
    return {"job_id": str(job.id), "status": job.status}
