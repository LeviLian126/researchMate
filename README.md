...
The repository is connected to the public `main` branch. After CI #4 exposed a case-sensitive documentation-path omission, the repair commit `c65aff3` completed GitHub Actions CI #5 successfully. This proves the remote quality gate only: GitHub Environments, cloud providers, OCI publication, migrations, smoke checks, rollback, and production deployment remain unvalidated.
