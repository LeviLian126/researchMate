...
使用 Node05 风险分类来选择能改变结论的域。表格是注意力图:深入检查每一行适用项,但不要仅因某域的表格存在就为其声称保证。
...
追踪攻击者可控输入到达的 sink,而非仅做模式匹配。对于每个候选问题,识别 source(攻击者可控输入)、缺失或被绕过的控制、sink(危险操作或受保护动作)和影响。一个模式如果没有可达的 source-to-sink 路径,就不是发现。

仅在有具体反证时才驳回候选问题:已针对确切输出上下文验证的框架自动转义、在每条到达 sink 的路径上都存在等效 guard、或代码在范围内未发布或不可达。一个安全的兄弟路径不能用来清除另一条不同路径。不要因为端点"本就执行危险操作"或仅凭名称看起来是内部的就驳回。

将 source-control-sink 追踪与最小安全可执行负向证明配对,只要该风险可达。优先使用现有针对性测试、最小回归测试或本地非破坏性复现。仅当执行确实不可用时才使用完整静态追踪,并将该限制记录为证明缺口,而非 PASS 等效证据。
...
| 检查项 | 要发现什么 | 严重级别 |
| --- | --- | --- |
| SQL 注入 | 使用字符串拼接 SQL 而非参数化查询;ORM raw query 包含用户输入 | Blocker |
...
| Archive 解压 | zip/tar 解压无路径包含检查;archive 中的 symlink 覆盖可信文件 | Blocker |
...
可靠性章节从工程属性角度覆盖幂等性和并发;本域从攻击者视角覆盖相同机制:用户能否利用它们绕过支付、配额或工作流约束。
...
| 重复支付 | 重试或并发提交创建重复扣费;验证幂等性和对账 | Blocker |
...
任何变更都不得引入密钥泄露、可达 XSS、已知高风险依赖或不安全传输。当变更文件、依赖图、生成的 bundle、部署配置或发布声明能影响该边界时,运行相应的仓库扫描或可执行检查。当变更触及 SSRF、支付/配额和 AI 表的输入、控制或 sink 时运行这些表。否则通过源码检查保持不变量,而不制造无关扫描。
...
| 重试与超时 | 网络或外部服务调用 | 有界 timeout,无无限 retry,存在退避策略 |
...
当其中某行适用时,将源码审查与能锻炼它的最小确定性失败证明配对——例如一次 timeout、格式错误或部分依赖响应、重复交付、受控的并发过渡、清理断言,或强制的输入/fan-out 限制。不要对生产环境或第三方系统运行破坏性 load、fuzz、chaos 或 DoS 实验。如果不存在安全的可执行边界,记录缺失证明以及获得它所需的 seam 或环境。
