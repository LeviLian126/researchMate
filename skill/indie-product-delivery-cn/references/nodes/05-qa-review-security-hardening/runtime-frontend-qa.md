...
通过能暴露真实集成和渲染失败的运行时边界,验证应用和受影响的用户旅程。从目标和变更面中选择旅程、状态、视口和环境;使用矩阵是为了聚焦观察,而非强制无关覆盖。

## 启动应用

- 启动 dev server 或运行生产 build。
- 确认无启动错误:无编译错误、无缺失依赖、无端口冲突、无应用所需环境变量缺失。
- 如果应用无法启动,则为 Blocker。进入下文的调试流程。
- 如果缺少所需运行时(例如数据库未安装、Redis 不可用),将启动标记为 NOT_RUN 并准确记录缺少什么。不要在无法启动应用时声称应用可用。
...
前端必须在其支持的设备类别上正确渲染。从布局契约中选择受影响的断点和相邻过渡。对于大规模前端重设计或预发布 QA,覆盖完整矩阵;对于窄范围组件变更,覆盖其目标宽度和两侧最近的断点。

### 6 级设备矩阵

| 级别 | 宽度 | 代表设备 | 选择条件 |
| --- | --- | --- | --- |
| XS 小型手机 | 320px | iPhone SE 1st gen, 较旧的 Android | 最小支持宽度或移动布局变化时 |
| S 标准手机 | 390px | iPhone 12-15, Samsung Galaxy S | 正常移动流程或触控/表单行为变化时 |
| M 大型手机 | 430px | iPhone Pro Max, Galaxy Note | 移动断点或密集内容变化时 |
| L 平板 | 768px | iPad 竖屏, Android 平板 | 侧边栏、抽屉、grid、table 或平板过渡变化时 |
| XL 小型笔记本 | 1280px | MacBook Air, 小型笔记本 | 桌面导航或多列布局变化时 |
| XXL 桌面 | 1920px | 标准显示器 | max-width、密度、宽屏数据或完整重设计验证 |

如果项目仅针对特定设备类别(例如仅移动端应用),你可以调整矩阵,但需说明原因,并至少覆盖目标类别的 XS、S 和 M。

### 每个级别的检查

在每个选定级别上运行适用的检查。标注 PASS 或 FAIL 并附截图或直接观察;记录被省略级别无法改变结论的原因。
...
任何选定必需级别上的布局重叠或溢出至少为 Major。核心路径上的同一问题(例如移动端主要操作按钮被重叠)为 Blocker。核心路径上的控制台错误或失败的网络请求为 Blocker。

## 调试运行时失败

当启动、旅程或响应式证明失败时,遵循此流程。不要同时进行多个推测性修复。
