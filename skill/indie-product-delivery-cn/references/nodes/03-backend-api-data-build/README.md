...
构建后端代码,使其业务行为、状态变化、失败边界和依赖关系在正常和不利条件下都保持可理解。实现应该通过构造方式保证健壮性,而不是依赖 Node05 在事后发现本可避免的缺陷。

根据目标和当前切片判断哪些关注点重要。下表将注意力聚焦于易错的实现面;并不要求为每次变更都构造每个机制。

| 实现面 | 期望属性 |
|---|---|
| 所有权与流程 | 每条规则只有一个清晰的所有者,显式的依赖,局部且可追踪的状态变化 |
| 契约与数据 | 验证不可信输入,稳定的输出,保留授权和持久不变式 |
| 失败与恢复 | 显式的领域适配失败语义,有界的 provider 行为,不吞没失败或制造虚假成功 |
| 并发与资源 | 在需要处实现幂等或协调的写入,有界的工作量,每条路径都有清理 |
| 可维护性 | 高内聚的模块,可读的控制流,无投机性抽象或重复策略 |
| AI 生成代码风险 | 核验已安装的 API 和签名;移除占位符、演示数据、虚构依赖和仅看似合理的逻辑 |
...
| 需求 | 阅读 |
|---|---|
| 框架化已批准的切片;还原实现真相;构建实现主干 | `slice-framing.md` |
| 实现领域行为、use-case 所有权、状态转换、策略 | `domain-build.md` |
