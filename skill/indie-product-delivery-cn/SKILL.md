...
或新会话。分两阶段运行验证,使 subagent 不继承实现者的假设。第一阶段:提供
目标、接口签名、schema、验收标准和风险分类,使其能在不看实现的情况下设计
契约优先的测试。第二阶段:提供实现 diff、第一阶段测试和 runtime 证据,使其
能审查源码并确认覆盖缺口。对于小型本地变更,默认不开启独立会话;完成适用的
本地检查和受影响的 server 检查即可。
