...
2. Verify each material fact against current evidence: capability and user impact,
   target environment, source ref/artifact, data/provider effects, Node05 status,
   required gates, recovery path, and support/watch need.
...
needed and keep the exploratory code on its pushed branch without forcing it into `main`.
