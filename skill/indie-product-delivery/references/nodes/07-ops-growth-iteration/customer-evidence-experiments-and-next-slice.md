...
#### 3. Execute the smallest experiment action

Node07 may execute the smallest action needed to test the current hypothesis when it is:
...
Never send spam, scrape private data, impersonate people, make unsupported claims, use dark
patterns, charge users, alter price/entitlement, or convert a manual trial into automation.
...
3. Route release actions to Node06, quality/ship status to Node05, and changed product truth
   to Node01.
