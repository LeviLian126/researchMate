...
the missing fact could change the skill's goal, trigger boundary, output needs, resource
...
Design around these questions, and state the goal's reason so the agent can choose the
appropriate precautions instead of mechanically following a checklist:
...
- **Context:** What facts, conventions, tools, and resources change the result, and in what
  order should they be consulted to avoid ambiguity, misunderstanding, or contradictions
  between contexts?
...
When writing or changing a skill, inspect its wording across the entrypoint and routed
resources for ambiguous terms, missing precedence, and statements that become contradictory
under another runtime or task context. Resolve those problems before adding more procedure.
Acquire context in the order that governs the result: user goal and authorization, repository
rules, current skill/resources, then only the references, scripts, assets, and external facts
...
Keep `SKILL.md` focused on the core workflow, routing, boundaries, and output principle. Put
...
After the first complete draft, use the runtime's independent subagent or fresh-session
self-check capability by default, using `agents/skill-auditor.md`. Give it the skill files and
the task-local goal, not the intended diagnosis or expected fixes. If that capability is
unavailable, perform the same audit from a clean context and state the limitation. It should
inspect whether the skill:
...
Let the goal determine which output details are necessary. Return enough observable evidence
for the user to tell whether the intended capability was created or improved, whether important
boundaries and context are clear, and whether the workflow is complete. Include changed paths,
validation or evaluation actually run, unresolved uncertainty, and the next useful action when
they affect that decision; omit ceremony that does not. Do not report a benchmark, user
preference, generated artifact, or external action that was not actually observed. Stop when the
goal's completion standard is evidenced, or clearly state what remains blocked.

## Final checks

Run the available skill validator. Check local links and resource routes. Inspect frontmatter,
trigger wording, output principle, and changed files. For a substantial skill, confirm that
