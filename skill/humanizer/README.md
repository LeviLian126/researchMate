...
A portable agent skill that improves naturalness, author consistency, factual fidelity, and
readability without changing meaning. It is plain Markdown, so it can run in any harness that
...
The skill includes an internal final audit pass and second rewrite to catch lingering machine-shaped prose. It normally returns only the final text; intermediate drafts and audit notes are available when requested.
...
The runtime skill keeps examples in the pattern references so the main behavior contract
stays small. Use the relevant pattern file when an example is needed; do not treat a sample's
facts, voice, or punctuation as a template for unrelated text.
...
- **2.9.2** - Made the internal audit the default for pasted, file, and embedded use; clarified priority between technical truth, user voice, repository conventions, and pattern cleanup; preserved already-good prose; broadened the skill to technical and agent-facing prose; removed the long runtime example.
