...
| --- | --- |
| preflight | existing rows, consumers, feature/config state, and required application permissions |
...
compatibility window. For large or unknown data, avoid a single unbounded write and
record the concrete lock/downtime/throughput risk for the later release workflow.

#### 5. Keep execution boundaries honest

Node03 may add migrations, safe preflight checks, dry-run modes, backfill code, repair
commands, fixtures, and local verification. Keep production migration, destructive cleanup,
and remote reconciliation in the Node06 release workflow; this keeps implementation and
...
| evidence | local fake, provider sandbox, signed fixture, or live confirmation |
...
| --- | --- |
| payment | server plan lookup, hosted flow, signature verification, event dedupe, sufficiently final entitlement state, and verified amount/recipient/provider state |
...
These are implementation safeguards. Keep secrets/API keys out of code and logs, and verify
money-related effects before treating them as successful.

#### 5. Prove behavior with truthful effects

Prefer the smallest appropriate evidence: adapter fake, signed callback fixture, local job
execution, dry-run import, sandbox, or live confirmation when the required secret/API key
is available and the money effect can be verified safely.
...
job, callback, or recovery behavior. Node05 handles independent quality review; Node06 handles
production reconciliation, rollout, monitoring, or rollback.
