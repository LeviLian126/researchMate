...
| sensitive slice | payment, admin, PII, tenant, auth, or destructive behavior changes | make the implementation boundary and core proof explicit |
...
When the same business decision appears in handlers, service branches, database triggers, and job code, identify the authoritative owner before adding another condition. Other layers may validate their boundary or translate the result, but they should not independently reinterpret the policy. This keeps state transitions testable and prevents behavior from diverging by entry point.
...
actually changed. Keep architecture decisions in Node02; leave independent quality review to
Node05.

## Keep transport and policy separate

Transport code parses, authenticates, validates shape, invokes the use case, and maps its stable result. It should not become a second policy engine. When HTTP, CLI, worker, or event entry points share a use case, keep their differences at the boundary and prove that authorization and state rules remain consistent across entry points.

## Prove consistency across entry points

When the same behavior is exposed through more than one transport, add one contract-level check that demonstrates the shared policy and result mapping. Avoid duplicating the entire test suite for every adapter.
