"""Persist and execute recoverable deletion of every artifact owned by a project."""
...
class SqlProjectDeletionStore:
    ...
    def mark_retryable(self, record: ProjectDeletionRecord, *, worker_id: str, code: str) -> None:
        with self.engine.begin() as connection:
            connection.execute(
                text(
                    """
...
class ProjectDeletionService:
    ...
    def handle(self, event: ProjectDeletionEvent, *, worker_id: str) -> str:
        """Delete remote artifacts for one claimed project deletion job."""
